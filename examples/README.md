...
The scenarios app (`src/scenarios`) builds standalone (`pnpm i && pnpm dev`). The `plugin-consumers/` demo references `@omena/eslint-plugin` + `@omena/stylelint-plugin`, which are **not yet published to npm** — that demo will install/lint once those packages ship. Lifted from the `omenien/omena-css` monorepo `examples/` tree.
